...
| `--surface-alt` | `#282828` | The raised band in the alternating section rhythm (`AlternatingSections` / `Section background="alt"`) — a step up from `--background`. White 14.7:1 and `--muted-foreground` 5.75:1 clear AA. **`--primary` is only 3.99:1 here** — pink is accent/large-text on this band, never small body text (it clears the 3:1 non-text/large-text floor, not the 4.5:1 body floor). Small pink labels that may land on it (the update `kind` label) sit on a `--card` chip instead (pink 5.36:1 on `#0a0a0a`, AA on every band). Surface only. |
...
| `--funding` | `#2BDE73` | Live-campaign accent: the "Currently Funding" cover badge and the "Fund it Here" button, both **black** text on the green (11.8:1; white fails at 1.8:1). Also used as green *text* on `--background` for the campaign "Ends …" line — 11.59:1, clears AA/AAA. |
| `--personalize` | `#2FA4B6` | Personalization secondary — teal marking a section as tuned to the signed-in person (the home "My Feed" rail; the `/me` dashboard band now uses `--dashboard`), used **in place of brand pink** when a section is personalized (§3 given a visual voice: pink = for everyone, teal = for you). A **black**-text surface (`--personalize-foreground` `#000000`, 7.11:1; **white fails at 2.96:1** — never white on it). As an accent (text/border/graphic) on `--background` it's 6.88:1 (clears AA). Replaced the former `--creator` pink and `--reader` cyan. The hero stays pink — it is deliberately never personalized. |
| `--dashboard` | `#2F9CFF` | The creator's private `/me` dashboard band only (distinct from `--personalize`, which stays on the home feed rail). A **black**-text surface (`--dashboard-foreground` `#000000`): black **7.32:1** (AAA), `black/70` 4.79:1 (AA); **white 2.87:1 FAILS** — never white on it. As an accent on `--background`, 7.07:1 (clears AA). `/me`-only by request; not a general personalization color. |
