...
export default async function ConventionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const meta = [convention.location, convention.whenHint].filter(Boolean).join(' · ')
  ...
  return (
    <Section as="article" padding="md" maxWidth="3xl" innerClassName="space-y-6">
      ...
      <header className="space-y-2">
        <h1 className="text-3xl font-black tracking-tighter uppercase">{convention.name}</h1>
        {meta && <p className="text-primary text-xs font-bold tracking-widest uppercase">{meta}</p>}
        {convention.description && (
          <p className="text-foreground/85 max-w-prose text-base">{convention.description}</p>
        )}
      </header>

      {convention.website && (
        <Button asChild size="lg" className="font-black tracking-wide uppercase">
          <a href={externalHref(convention.website)} target="_blank" rel="nofollow noopener noreferrer">
