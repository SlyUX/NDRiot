...
export async function postUpdate(
  ...
  // Keep only mention ids that are real comics/creators/conventions/media — no
  // dangling refs from a tampered form.
  let validMentions: string[] = []
  if (mentionIds.length) {
    try {
      validMentions = await client.fetch<string[]>(
        `*[_type in ["creator","book","convention","media"] && _id in $ids]._id`,
        { ids: mentionIds },
      )
    } catch (cause) {
