...
export async function POST(request: Request) {
  ...
  if (!doc || !type || !id || id.startsWith("drafts."))
    return Response.json({ ok: true });

  if (type === "creator") {
    if (await wasNotified("creator-live", id))
      return Response.json({ ok: true, skipped: "already" });
    const email = await ownerEmailOf(id);
    if (!email) return Response.json({ ok: true, skipped: "no-owner" });
    const ok = await sendCreatorLiveEmail({
      to: email,
      name: str(doc.name),
      slug: str(rec(doc.slug)?.current) || null,
    });
  ...
  if (type === "book") {
    if (await wasNotified("book-live", id))
      return Response.json({ ok: true, skipped: "already" });
