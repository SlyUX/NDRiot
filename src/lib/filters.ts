...
/**
 * Conventions filter by US state — a **State** dropdown. The options carry state
 * NAMES (what a reader reads); the stored value is the CODE, so conventionFilters
 * maps name→code. State-only for now (city is free text on both sides, and
 * profile-based "near me" is deferred until we add real geolocation).
 */
