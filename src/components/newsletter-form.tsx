...
export function NewsletterForm({
  ...
  // Instance-unique ids so the same variant can render more than once on a page
  // (the home band shows in two positions, toggled by breakpoint).
  const uid = useId();
  const errorId = `nl-error-${uid}`;
  const consentId = `nl-consent-${uid}`;
  const companyId = `nl-company-${uid}`;
  ...
  return (
    <form action={action} className={cn("w-full", className)} noValidate>
      ...
      {state.status === "error" && (
        <p
          ...
          className={cn(
            ...
            onPink ? "text-black" : "text-destructive",
          )}
        >
      ...
      <p
        ...
        className={cn(
          "mt-2 text-xs",
          onPink ? "text-black" : "text-muted-foreground",
        )}
      >
        {copy.consent}
      </p>
