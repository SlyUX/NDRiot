...
export async function submitCreator(
  ...
  if (!values.name)
    fieldErrors.name = "Please add the name you want to be credited by.";
  ...
  if (!isYes(String(formData.get("permission") ?? "")))
    fieldErrors.permission =
      "We can only list work you confirm you have the right to share.";
  ...
  if (values.feedUrl && !feedUrl)
    fieldErrors.feedUrl = "That doesn’t look like a valid web address.";
  ...
  try {
    ...
    return {
      status: "error",
      message:
        "On-site submissions are temporarily unavailable — please use the form linked below.",
      values,
    };
  }
  ...
  if (isUpdate && !(await ownsCreator(email, target!._id))) {
    return {
      status: "error",
      message: "You can only edit a profile you own.",
      values,
    };
  }

  // One profile per Google account. A NEW submission from an email that already
  // owns a creator profile is refused here — the server backstop for the UI
  // guardrail — so duplicate profiles (which we can't yet cleanly reconcile)
  // never get created. Editing the one you have is always fine.
  if (!isUpdate && (await creatorsOwnedBy(email)).length > 0) {
    return {
      status: "error",
      message:
        "Your account already has a creator profile — you can edit it, but not create a second. To make a separate profile, sign out and use a different Google account.",
      values,
    };
  }
  ...
  try {
    ...
    takenIds = new Set(
      (creatorIds ?? []).map((id) => id.replace(/^drafts\./, "")),
    );
  } catch (cause) {
  ...
  const selectedOrgIds = [
    ...new Set(formData.getAll("orgs").map(String)),
  ...
  const genres = matchTaxonomy(
    formData.getAll("genres").map(String),
    GENRES,
  ).matched.slice(0, 3);
  const formats = matchTaxonomy(
    formData.getAll("formats").map(String),
    FORMATS,
  ).matched;
  ...
  if (orgIds.length) {
    fields.organizations = orgIds.map((id) => ({
      _type: "reference",
      _key: id,
      _ref: id,
    }));
...
async function notifyTeam(input: {
  ...
  if (input.photoNote)
    lines.push(``, `Heads up: their photo didn’t upload — ${input.photoNote}`);
