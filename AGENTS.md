...
| `--creator` | `#AA336A` | Secondary brand — the darker pink for creator/owner zones (the `/me` creator profile section background). A **white**-text surface (`--creator-foreground` `#FFFFFF`, 6.19:1; black fails at 3.39:1). Surface/fill only — as *text* on `--background` it's 3.33:1, so never body text on dark. |
| `--reader` | `#00FEFF` | Base-reader secondary — electric cyan, the reader counterpart to `--creator` (e.g. the homepage feed rail's user-type accent). **Accent only** on `--background` — text/border/graphic is 16.2:1 (clears AAA). The inverse of `--creator`: never a fill (a cyan fill needs **black** text, 16.6:1; white fails at 1.26:1). |
